...
def create_audio(stories):
    for story in stories:
        ...
        try:
            ...
            command = AudioSegment.from_file(f"commands/{story}.mp3", format="mp4")
        parent_says = AudioSegment.from_file(f"{story}/parent_says.m4a", format="mp4")
        ...
        #third slide
        parent_wants = AudioSegment.from_file(f"{story}/parent_wants.m4a", format="mp4")
        aligned = AudioSegment.from_file(f"{story}/goals_aligned.m4a", format="mp4")
        misaligned = AudioSegment.from_file(f"{story}/goals_misaligned.m4a", format="mp4")
        aligned_combined = parent_wants + aligned
        misaligned_combined = parent_wants + misaligned
        
        # #fourth slide
        intro = AudioSegment.from_file(f"{story}/options_intro.m4a", format="mp4")
        compliance = AudioSegment.from_file(f"{story}/compliance.m4a", format="mp4")
        loophole = AudioSegment.from_file(f"{story}/loophole.m4a", format="mp4")
        noncompliance = AudioSegment.from_file(f"{story}/noncompliance.m4a", format="mp4")
        or_audio = AudioSegment.from_file("or.mp3", format="mp4")
        or_2 = AudioSegment.from_file("or_2.m4a", format="mp4")
        options_combined = intro + compliance + loophole + or_2 + noncompliance
        options_audio = {'c': compliance, 'l': loophole, 'nc': noncompliance}
        
        for perm in options_permutations:
            order = "-".join(perm)
            audio = intro + options_audio[perm[0]] + options_audio[perm[1]] + or_2[200:-200] + options_audio[perm[2]]
